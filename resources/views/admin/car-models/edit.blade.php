                                ...
                                <div class="form-group">
                                    ...
                                    <select name="years[]" id="years" class="form-control select2-tags" multiple="multiple" data-tags="true">
                                        ...
                                        @foreach($allYears as $year)
                                            <option value="{{ $year }}" {{ in_array($year, $existingYears) ? 'selected' : '' }}>
                                                {{ $year }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="types[]" id="types" class="form-control select2-tags" multiple="multiple" data-tags="true">
                                        @php
                                            ...
                                            // تیپ‌های پیش‌فرض
                                            $defaultTypes = ['صندوق دار', 'سدان', 'هاچ بک', 'شاسی بلند', 'کوپه', 'ون', 'پیکاپ', 'کراس اوور', 'مینی ون', 'اسپورت'];
                                            // ترکیب تیپ‌های موجود و پیش‌فرض
                                            $allTypes = array_unique(array_merge($defaultTypes, $existingTypes));
                                        @endphp
                                        @foreach($allTypes as $type)
                                            <option value="{{ $type }}" {{ in_array($type, $existingTypes) ? 'selected' : '' }}>
                                                {{ $type }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <small class="form-text text-muted">می‌توانید تیپ‌های جدید را تایپ کرده و Enter بزنید</small>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="colors[]" id="colors" class="form-control select2-tags" multiple="multiple" data-tags="true">
                                        @php
                                            ...
                                            // رنگ‌های پیش‌فرض
                                            $defaultColors = [
                                                'مشکی', 'سفید', 'نقره‌ای', 'خاکستری', 'قرمز', 'آبی',
                                                'سبز', 'زرد', 'نارنجی', 'صورتی', 'بنفش', 'قهوه‌ای',
                                                'طلایی', 'نوک مدادی', 'بژ', 'فیروزه‌ای', 'یاسی', 'زرشکی'
                                            ];
                                            // ترکیب رنگ‌های موجود و پیش‌فرض
                                            $allColors = array_unique(array_merge($defaultColors, $existingColors));
                                        @endphp
                                        @foreach($allColors as $color)
                                            <option value="{{ $color }}" {{ in_array($color, $existingColors) ? 'selected' : '' }}>
                                                {{ $color }}
                                            </option>
                                        @endforeach
                                    </select>
